function r=FindIndex(NR)
%FindIndex determine the array of indices of Bounding boxes of interest.
%   R=FindIndex(NR) outputs the row vector R containing the indices of
...
%% cancatenate the new column
NR2=cat(2,NR,C_5');           % Appending new coloumn in NR.
%% counter and center for 5th columns
[E,R]=hist(NR2(:,5),20);
%% searching
Y=find(E==7);                 % Searching for six characters.
%% make container for the character
...
%%  calling the function
    r=takeboxes(NR,container,2); % call function
% title('bounding boxes around the objects');
% for idx = 1: numel(NR) 
%     if size(NR,1)==r
%     rectangle('Position', NR(idx,:), 'edgecolor', 'green');
%     end
% end
%%
elseif length(Y)==1
    ...
    %%
elseif isempty(ind) || length(ind)>1 % If there is no vlaue of '7' in the Q vector.
    ...
    if length(ind2)==1
        ...
        container=guessing(A,B,(B(2)-B(1))); % Call of function guessing.
        if ~isempty(container) % If guessing works succesfully.
            r=takeboxes(NR,container,1); % Call the function takeboxes.
        elseif isempty(container)
            container2=guessing(E,R,(R(2)-R(1)));
